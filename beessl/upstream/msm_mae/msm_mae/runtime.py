...
def parse_sizes_by_name(name):
    # Regex pattern to find the required numbers
    pattern = r'(\d+)x(\d+)(?:p(\d+)x(\d+))?'
    matches = re.findall(pattern, name)
    first_list, second_list = [], []

    for match in matches:
        # The first two groups are always present
        first_list.append(int(match[0]))
        first_list.append(int(match[1]))
        
        # The third and fourth groups are optional (may be present after 'p')
        if match[2] and match[3]:  # If these groups are present, add them to the second list
            second_list.append(int(match[2]))
            second_list.append(int(match[3]))
